...
class SingleImageDataModuleConfig:
    ...
    json_path: str = "./load/multi/data.json"


class SingleImageDataBase:
    def setup(self, cfg, split, fix_camera=False):
        self.split = split
        self.rank = get_rank()
        ...
        if self.cfg.use_random_camera:
            ...
            if split == "train":
                ...
                self.random_pose_generator = RandomCameraDataset(
                    random_camera_cfg, split, fix_camera
                )
    ...
    def load_images(self):
        ...
        rgba = Image.open(self.cfg.image_path).convert('RGBA')
       
        # with open(self.cfg.json_path, 'r') as json_file:
        #     data = json.load(json_file)
        #     filename = os.path.basename(self.cfg.image_path)
            
        #     if filename in data:
        #         x_offset, y_offset = data[filename]
        #         tmp = Image.new('RGBA', rgba.size, (0, 0, 0, 0))
        #         tmp.paste(rgba, (x_offset, y_offset), rgba)
        #         rgba = tmp
        
        rgba = rgba.resize((self.width, self.height))
        
        # Convert the resized RGB image to a NumPy array
        rgba = np.array(rgba).astype(np.float32) / 255.0
...
class SingleImageDataset(Dataset, SingleImageDataBase):
    def __init__(self, cfg: Any, split: str, fix_camera: bool = False) -> None:
        super().__init__()
        self.setup(cfg, split, fix_camera)
...
class SingleImageDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None) -> None:
        ...
        if stage in [None, "test", "predict"]:
            self.test_dataset = SingleImageDataset(self.cfg, "test", self.fix_camera)
